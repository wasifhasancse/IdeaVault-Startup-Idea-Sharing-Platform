...
const CheckboxGroup = ({ items, name, cols = 3, defaultValues = [] }) => (
    ...
    {items.map((item) => (
      ...
      >
        <input
          ...
          value={item}
          defaultChecked={
            Array.isArray(defaultValues) && defaultValues.includes(item)
          }
          className="sr-only"
        />
...
export default function UpdateForm({ ideasDetails }) {
  ...
  const [state, formAction] = useActionState(
    (prevState, formData) => UpdateIdeasAction(formData, _id),
    null,
  );
  useEffect(() => {
    if (state?.success) {
  ...
  return (
    <form action={formAction} className="flex flex-col gap-5">
      ...
      <div className="rounded-xl border border-[#5e41de]/10 bg-[#5e41de]/3 px-4 py-3 dark:border-[#5e41de]/15 dark:bg-[#5e41de]/5">
        ...
        <CheckboxGroup items={TAGS} name="tags" cols={3} defaultValues={tags} />
      ...
      <div className="rounded-xl border border-[#5e41de]/10 bg-[#5e41de]/3 px-4 py-3 dark:border-[#5e41de]/15 dark:bg-[#5e41de]/5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          {/* Estimated Budget */}
          <div className="flex flex-col gap-1.5">
            <FieldLabel icon={FiDollarSign}>Estimated Budget (USD)</FieldLabel>
            <input
